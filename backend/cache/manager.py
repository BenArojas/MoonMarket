...
class CacheManager:
    ...
    async def set_historical_stock_data_for_ticker(self, ticker: str, time_range: str, metrics: tuple, data: dict, ttl_seconds: int) -> None:
        """Set historical stock data for a single ticker in Redis cache."""
        cache_key = f"stock:historical:{ticker}:{time_range}:{','.join(metrics)}"
        await self.redis.setex(cache_key, ttl_seconds, json.dumps(data))