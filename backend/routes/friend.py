...
async def get_pending_friend_requests_length(
    current_user: User = Depends(get_current_user),
):
    pending_requests = await FriendRequest.find(
        FriendRequest.to_user.id == current_user.id, FriendRequest.status == "pending"
    ).to_list()
...
async def get_pending_friend_requests_users(
    current_user: User = Depends(get_current_user),
):
    pending_requests = await FriendRequest.find(
        FriendRequest.to_user.id == current_user.id, FriendRequest.status == "pending"
...
async def get_sent_friend_requests(current_user: User = Depends(get_current_user)):
    pending_requests = await FriendRequest.find(
        FriendRequest.from_user.id == current_user.id, FriendRequest.status == "pending"
    ).to_list()
    to_users = []
    for request in pending_requests:
...
async def handle_friend_request(
    ...
    if to_user.id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Not authorized to handle this friend request"
        )
...
async def get_all_friends(
    ...
    for user_id in users:
        ...
        for holding in user.holdings:
            ...
            if not stock:
                stock = await cache_manager.get_stock_info_for_ticker(holding.ticker)
                if not stock:
                    stock = await fetch_stock_data_yf(holding.ticker)
                if stock:
            ...
            if stock:
                ...
                portfolio_percentage = (
                    (holding_value / holdings_value) * 100 if holdings_value > 0 else 0
                )
        ...
        portfolio_percentage_change = (
            ((holdings_value - cost_basis) / cost_basis) * 100
            if cost_basis > 0
            else (0 if holdings_value == 0 else 100)
        )
...
async def remove_friend(
    friend_id: str, request: Request, current_user: User = Depends(get_current_user)
):
    ...
    if friend_object_id not in current_user.friends:
        raise HTTPException(
            status_code=404, detail="Friend not found in your friend list"
        )
