        ...
        switch (step) {
            ...
            case 2: // Account-specific setup
                if (apiProvider === 'fmp') {
                    ...
                    return ( // IBKR Connection Step
                        <>
                            <AlertDialogHeader>
                                <AlertDialogTitle className="flex items-center"><Rocket className="mr-2 h-6 w-6 text-customTurquoise-400" />IBKR Astrogation Link</AlertDialogTitle>
                                <AlertDialogDescription className="text-sm space-y-2">
                                    <p>Prepare for hyperspace jump! You're connecting directly to the Interactive Brokers galaxy.</p>
                                    <p>This secure OAuth 2.0 process will link your IBKR account to MoonMarket:</p>
                                    <ol className="list-decimal list-inside ml-4 space-y-1">
                                        <li>Click "Engage IBKR Hyperdrive" below.</li>
                                        <li>You'll be transported to the official IBKR portal.</li>
                                        <li>Log in and authorize MoonMarket to access necessary data (we only request what's needed for your portfolio!).</li>
                                        <li>IBKR will then send you back to MoonMarket.</li>
                                    </ol>
                                    <p>Once the link is established, you can proceed to the next step.</p>
                                    {isIbkrSuccessfullyConnected && (
                                        <p className="text-green-400 font-semibold mt-2">Great Scott! IBKR Hyperdrive connection established!</p>
                                    )}
                                </AlertDialogDescription>
                            </AlertDialogHeader>
                            {!isIbkrSuccessfullyConnected && (
                                <Button
                                    onClick={handleInitiateIbkrOAuth}
                                    className="w-full my-4 bg-blue-600 hover:bg-blue-700 text-white"
                                >
                                    <Rocket className="mr-2 h-4 w-4" /> Engage IBKR Hyperdrive
                                </Button>
                            )}
                            {isIbkrSuccessfullyConnected && (
                                 <p className="my-4 p-3 bg-green-900/50 text-green-300 border border-green-700 rounded-md text-center">
                                    IBKR connection active! You're cleared for the next phase.
                                </p>
                            )}
                            <AlertDialogFooter>
                                <Button variant="outline" onClick={() => setStep(1)} className="hover:border-customTurquoise-400/50">Back to Spacecraft Selection</Button>
                                <AlertDialogAction asChild>
                                    <Button
                                        onClick={handleProceedFromIbkr}
                                        disabled={!isIbkrSuccessfullyConnected}
                                        className="bg-customTurquoise-400 text-white hover:bg-customTurquoise-400/80"
                                    >
                                        Next: Set Parameters
                                    </Button>
