import { fetchWatchlistDetail, fetchWatchlistHistoricalData } from "@/api/watchlist";
import { useDebounce } from "@/hooks/useDebounce";
import { PortfolioPerf, stimulatedPortfolioItem, watchListStockData } from "@/types/watchlist";
...
import { CenteredBox } from "./Watchlist";
...
interface WatchlistContentProps {
  ...
  export const WatchlistContent: React.FC<WatchlistContentProps> = ({ selectedId }) => {
    ...
    const { data: watchlistDetail, isPending: detailIsPending } = useQuery({
      ...
      queryFn: () => fetchWatchlistDetail(selectedId),
    });
  
    const idealTickers = useMemo(() => watchlistDetail?.instruments?.map((i: any) => i.ticker) ?? [], [watchlistDetail]);
    ...
    const uniqueTickers = Array.from(
        new Set([...debouncedComparisonTickers, benchmark])
      );
    ...
    const {
      ...
      error: pricesError,
      fetchStatus,
    } = useQuery<watchListStockData[]>({
      queryKey: ["prices", debouncedComparisonTickers, benchmark, timeRange, selectedId], 
      enabled: debouncedComparisonTickers.length > 0 || !!benchmark,
      queryFn: () => fetchWatchlistHistoricalData(uniqueTickers, timeRange, secTypes),
    });
  
    /* --- 3. Data Reliability & Sync Logic --- */
    ...
    // This effect removes tickers from the multiselect if the API fails to return data for them.
    useEffect(() => {
      if (availableTickers.length > 0) {
        setComparisonTickers(prev => prev.filter(ticker => availableTickers.includes(ticker)));
      }
    }, [availableTickers.join(',')]); // Depend on the string value to avoid re-renders
  
    // This effect initializes quantities when the ideal tickers from the watchlist change.
    useEffect(() => {
      const init: Record<string, number> = {};
      idealTickers.forEach((t) => (init[t] = 0));
      setLocalQuantities(init);
    }, [idealTickers.join(',')]); // Depend on the string value
  
  
    /* --- 4. Derived State for Child Components --- */
    const comparisonChartData = useMemo(() => {
      ...
      const priceMap = new Map(reliableStocksData.map(stock => [stock.ticker, new Map(stock.historical.map(p => [p.date, p.price]))]));
    ...
    const watchlistPortfolio: stimulatedPortfolioItem[] = useMemo(() => Object.entries(localQuantities).filter(([_, qty]) => qty > 0).map(([ticker, quantity]) => ({ ticker, quantity })), [localQuantities]);
    ...
    const processPortfolioPerformanceChartData = useCallback(
        (data: watchListStockData[], portfolio: stimulatedPortfolioItem[], benchmarkTicker: string) => {
    ...
    return (
      <>
        ...
        {!pricesPending && !pricesHaveError && (
          <>
            <ComparisonChart chartData={comparisonChartData} benchmark={benchmark} />
            <WatchlistTable watchlist={idealTickers} stocksData={reliableStocksData} timeRange={timeRange} localQuantities={localQuantities} handleQuantityChange={handleQuantityChange} />
            <PortfolioSummary portfolioPerformance={portfolioPerf} portfolioChartData={portfolioChartData} timeRange={timeRange} benchmark={benchmark} />
          </>
        )}
      </>
    );
